                                                    ...
                                                    DefArc& smoothArc)
{
    ...
    gocal.IsIdenticalPoint(smoothArc.GetSp(),smoothArc.GetEp(),
    ...
    arcrot.ArcRotDirBaseOriginLine(l1Sp,smoothArc.GetSp(),true,
                                   smoothArc,smoothArc._isCW);
    isExiSmooth = true;
}
                                            ...
                                            DefArc& smoothArc)
{
    ...
    ///计算原始圆弧arc1 所在圆与连接centerPnt 与圆弧圆心的段的交点
    Point cirIntPnt1;
    GeometryLogic golg;
    GeomCalculate gocal;
    GeomIntersect goInt;
    ...
    if(gocal.TwoPointDistance(arc1.GetEp(),center) > strechLimit){
        return;
    }
    golg.ClosetPointFromTwoPoint(center,intRes1._intPnt1,
                                 intRes1._intPnt2,cirIntPnt1);
    ///计算原始圆弧arc2 所在圆与连接centerPnt 与圆弧圆心的段的交点
    Point cirIntPnt2;
    ...
    arcrot.ArcRotDirBaseOriginArc(temArc,true,smoothArc,smoothArc._isCW);
    /*crtArc_go->TriangleRotDirection(newArc._arcCp,newArc._arcSp,
                                 newArc._arcEp,newArc._isCW);*/
    isExiSmooth = true;
}
