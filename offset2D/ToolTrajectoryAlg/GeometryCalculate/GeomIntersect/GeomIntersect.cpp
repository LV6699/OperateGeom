...
#pragma optimize("", off)
#endif
void GeomIntersect::CalCircleCircleExiIntPnt(ofts::Point c1,double r1,
                                             ofts::Point c2,double r2,
                                             ofts::Point& p1,ofts::Point& p2)
{
    if(r1 > r2){std::swap(c1,c2);std::swap(r1,r2);}
...
{
    CurveBoxInt boxint;
    if(!boxint.IsRectExistInt(lSp1,lEp1,lSp2,lEp2,
                              intPre * ErrTimes_10))
        return;
    ...
    if(vIntPnt.empty()){
        ...
        CalculateLineLineInt(lSp1,lEp1,lSp2,lEp2,isExiLLInt,intPnt);
        if(isExiLLInt){
                                                ...
                                                GeomIntRes& intRes)
{
    PointOnElement pton;
    bool isSpOn = false,isEpOn = false;
    pton.IsPtOnCircle(lSp,cp,R,endptPre,isSpOn);
    pton.IsPtOnCircle(lEp,cp,R,endptPre,isEpOn);
    Point onEndpnt;
    bool isEndpntOn = false;
                                          ...
                                          GeomIntRes &intRes)
{
    ...
    PointOnElement pton;
    bool isSpOn = false,isEpOn = false;
    pton.IsPtOnCircle(arc._arcSp,cp,R,endptPre,isSpOn);
    pton.IsPtOnCircle(arc._arcEp,cp,R,endptPre,isEpOn);
    Point onEndpnt;
    bool isEndpntOn = false;
    GeomIntSolve().ExicuteEndpointOnTuple(isSpOn,isEpOn,arc._arcSp,arc._arcEp,
                                          isEndpntOn,onEndpnt,intRes);
                                           ...
                                           GeomIntRes &intRes)
{
    ...
    if(intRes_._intNum == 2){
        pton.IsPointOnSegment(lSp,lEp,intRes_._intPnt2,endptPre,
                              intPre,false,isPnt2OnSig);
        if(isPnt2OnSig){
            pton.IsPointOnArc(arc,intRes_._intPnt2,endptPre,
                              intPre,false,isPnt2OnArc);
            if(isPnt2OnArc){
                                       ...
                                       double intPre,GeomIntRes& intRes)
{
    ...
    CalculateCircleCircleInt(arc1._arcCp,arc1._arcR,arc2._arcCp,
                             arc2._arcR,tanPre,intPre,intRes_);
    if(!intRes_._isExistInt && !intRes__._isExistInt){
        return;
    }
