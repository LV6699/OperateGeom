...
{
    Point normal = s.Normal().Normalized(); ///建立局部坐标系
    /// 创建平面内的第一个基向量 - 任意选择一个不平行于法向量的向量
    Point basis1;
    if (std::abs(normal.X()) > 0.5) {  /// 如果法向量主要指向x 方向
        basis1 = Point(0, 1, 0);
    } else {
        basis1 = Point(1, 0, 0);
    }
    /// 投影到平面内并归一化
    basis1 = (basis1 - normal * normal.Dot(basis1)).Normalized();
    /// 创建第二个基向量（与第一个正交）
    Point basis2 = normal.Cross(basis1).Normalized();
    ps.push_back(p);  /// 顶点1: p (给定的点)
    ps.push_back(p + basis1 * l);  /// 顶点2: p + basis1 * l
    ps.push_back(p + basis1 * l + basis2 * l);  /// 顶点3: p + basis1 * l + basis2 * l
    ps.push_back(p + basis2 * l);  /// 顶点4: p + basis2 * l
}
void CreateCurveCase::ThreePointPlan(const Point& p0, const Point& p1,
                                     const Point& p2, DefPlane& plan,double e)
{
#if 0
...
#endif
    if(GeomCalculate().ThreePointCollinear(p0,p1,p2,e)){
        throw std::invalid_argument("共线3 点无法创建平面");
        return;
    }
    Point v1 = p1 - p0;
    Point v2 = p2 - p0;
    Point n = v1.Cross(v2);  ///计算法向量
    double D = -n.Dot(p0);   ///计算D 值
...
#endif
    if(GeomCalculate().ThreePointCollinear(p0,p1,p2,e)){
        throw std::invalid_argument("共线3 点无法创建法平面");
        return;
    }
    Point v1 = p1 - p0,v2 = p2 - p0;  ///该平面s 经过点p0.
    Point n1 = v1.Cross(v2);  ///计算原始平面的法向量
    Point n2 = n1.Cross(v1);  /// 选择v1 作为方向向量计算垂直平面的法向量
    n2 = n2.Normalized();
    double D = -n2.Dot(p0);
