...
public:
    ...
    bool IsLoopExistNexEle(const DefLoop&loop,int i)
    {
        if(loop.m_vElem.empty())
            return false;
        if(!loop.IsOpen() || i != loop.m_vElem.size() - 1)
            return true;
        return false;
    }
    void DeduplicatePts(vector<Point>& points,double err){
        std::vector<size_t>ids;
        for (size_t i = 0; i < points.size(); ++i) {
            for (size_t j = i+1; j < points.size(); ++j) {
                if(points[i].IsSamePoint2D(points[j],err)){
                    ids.push_back(i);
                    break;
                }
            }
    ...
    void DeleIllegalCurveNumLoop(vector<DefLoop>&vLoop);
    bool IsElementIndexValid(const DefLoop&loop,int i);
    ...
    void GetNodeLoopNum(const std::shared_ptr<OffsetNode>&rootNode,int& num);
    void GetNodeEleNum(const std::shared_ptr<OffsetNode>&rootNode,int& num);
    void GetNodeEleLenth(const std::shared_ptr<OffsetNode>&rootNode,double& len);
    ...
    void DeleteIndexLoop(const vector<size_t> &vIndex, vector<DefLoop>& vLoop);
    void DeleteIndexElement(const vector<size_t> &vIndex, vector<DefElem>& vElem);
    ...
    void PointSplitElement(const DefElem& ele,
                           const Point& P,
                           bool isChEndPnt,
                           DefElem& newEle);
    ...
    DefCircle PointsMinCircumcircle(vector<Point>& points,
