                                          ...
                                          int & eleIndex)
{
    ...
    for (int lo = 0; lo < vLoop.size(); ++lo) {
        if(lo == curLoIndex || !vLoop[lo]._label._isIntersect){
            continue;
        }
                                         ...
                                         vector<DefLoop>& vResLoop)
{
    ...
    ElemCalculate numcal;
    int hasOftNum = vLoop[0].PastOftNum();
    for (int i = 0; i < vArrIndex.size(); ++i) {
        if(vArrIndex[i].empty())
            continue;
        DefLoop loop;
        bool isExistClip = false;
        for(int lo = 0;lo < vArrIndex[i].size();lo ++){
            int nex = numcal.NexId(vArrIndex[i].size(),lo);
        ...
        loop._label._isIntersect = isExistClip;
