...
void BaseMinPassage::ExploreProbableUnprocess(const DefLoop &loop,
                                              const double R, const double oftValue,
                                              const int index,IndexDist &idDistance)
{
    if(index >= loop.Size() - 2)
        return;
    GeomDistance godis;
    vector<ValidDist>vIndexDist;
    vIndexDist.resize(loop.Size());
    int i = loop.NexId(index);
    i = loop.NexId(i);
    ...
    for (int i = 0; i < idDistance._vIndexDist.size(); ++i) {
        ...
        DefElem ele_ = loop.IndexElem(idDistance._vIndexDist[i]._indexDist.first);
        if(ele_.Type() == ARCTYPE){
            ele_._arc.OptInitR(PreErr5_2);
            arcStep = (k / (2 * M_PI * ele_._arc.R())) * 360;
        }
        else{
