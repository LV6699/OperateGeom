...
public:
    ...
    string PointToString(const ofts::Point& p,int pre = 18);
    void EnumSwitchString(bool iscw, std::string& str);
    ...
    void PathNodeString(const std::shared_ptr<OffsetNode>& pathNode,
                        int& n,string& str);
    void RegionssString(const std::vector<std::vector<ofts::GeomArea>>& regionss,
                        string &str);
    bool SaveStringToFile(const std::string& str, const std::string& path);
};
