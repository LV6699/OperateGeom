                                             ...
                                             bool& isExiInt)
{
    ...
    if(isExiInt)
        return;
    pton.IsPointOnSegment(lSp2,lEp2,lEp1,pre,pre,true,isExiInt);
    if(isExiInt)
        return;
                                            ...
                                            bool& isExiInt)
{
    isExiInt = false;
    PointOnElement pton;
    pton.IsPtOnCircle(lSp,cp,R,pre,isExiInt);
    if(isExiInt)
        return;
    pton.IsPtOnCircle(lEp,cp,R,pre,isExiInt);
    if(isExiInt)
        return;
    GeomIntRes intRes;
    GeomIntersect goInt;
                                        ...
                                        bool& isExiInt)
{
    isExiInt = false;
    PointOnElement pton;
    pton.IsPtOnCircle(arc._arcSp,cp,R,pre,isExiInt);
    if(isExiInt)
        return;
    pton.IsPtOnCircle(arc._arcEp,cp,R,pre,isExiInt);
    if(isExiInt)
        return;
    JudgeArcRadius(arc);
    GeomIntRes intRes;
                                         ...
                                         bool& isExiInt)
{
    ...
    if(intRes._intNum == 2){
        ...
        if(isPnt2OnSig){
            pton.IsPointOnArc(arc,intRes._intPnt2,pre,
                              pre,false,isPnt2OnArc);
            if(isPnt2OnArc)
                isExiInt = true;
        }
    }
...
#else
        tbb::parallel_for(tbb::blocked_range<std::size_t>(0, loop2.Size()),
                          [&](tbb::blocked_range<std::size_t>& r) {
