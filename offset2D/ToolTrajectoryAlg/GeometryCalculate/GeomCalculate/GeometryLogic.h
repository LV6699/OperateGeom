...
public:
    GeometryLogic(){}
    void IsLineCircleTan(const Point& lSp,
                         const Point& lEp,
                         const Point& CP,
                         double R, double pre,
                         bool& isTan);
    ...
    void CloseArcEndpointBaseLenth(DefArc arc,
                                   ...
                                   double stepVal,
                                   Point &P);
    void ApproachCurveEndpoint(const DefElem& ele,double stepVal,
    ...
    void TranslateBndIslToSegment(double stepVal,
                                  bool isStepAngle,
                                  GeomArea& BILoop);
    void ThirdOrderBezierCurve(vector<Point>& vControlPt,
                               DefLoop& loop);
    ///从loop 首元素开始正向(起点指向终点方向)计算.
    void LoopSpecifyLenthPosition(const DefLoop& loop,
                                  double lenth, int &index, Point& P);
    void PointToSegmentLoop(const vector<Point>&points, bool isClose, DefLoop& loop);
};
