                                        ...
                                        bool& isReorderEle)
{
    ...
    for (int i = 0; i < loop.m_vElem.size(); ++i) {
        ...
        double lenth;
        gocal.TwoPointDistance(loop.m_vElem[i]._segment.GetSp(),
                               loop.m_vElem[i]._segment.GetSp(),
                               lenth);
        ...
        Point midPnt;
        GeomCalculate gocal;
        gocal.TwoPointMidpoint(loop.m_vElem[i]._segment.GetSp(),
                               loop.m_vElem[i]._segment.GetEp(),
                               midPnt);
        DefElem ele1(LINETYPE,DefSeg(midPnt,loop.m_vElem[i]._segment.GetEp()));
        DefElem ele2(LINETYPE,DefSeg(loop.m_vElem[i]._segment.GetSp(),midPnt));
        if(isReorderEle){
    ...
    for (int i = 0; i < loop.m_vElem.size(); ++i) {
        if(loop.m_vElem[i]._type != ARCTYPE){
            continue;
        }
        return;
    }
...
{
    GeomCalculate gocal;
    if(element._type == LINETYPE){
        gocal.SegmentAngle(element._segment.GetSp(),
                           element._segment.GetEp(),slope);
        return;
    }
    Point rotPt;
    if(element._type == ARCTYPE){
        gocal.PointAroundFixedRotate(P,element._arc.GetCp(),
                                     90,element._arc.IsCW(),rotPt);
        gocal.SegmentAngle(element._arc.GetCp(),
                           rotPt,slope);
    }
    else if(element._type == CIRCLETYPE){
        gocal.PointAroundFixedRotate(P,element._circle.GetCp(),
                                     90,rotDir,rotPt);
        gocal.SegmentAngle(element._circle.GetCp(),
                           rotPt,slope);
    }
}
                                        ...
                                        double l,Point& extpt)
{
    ...
    double agl;
    bool iscw = true;
    if(curve.Type() == OffsetEnum::CIRCLETYPE && !curve._circle.IsCW()){
        iscw = false;
    }
    PointOnElementAngle(curve,pos,iscw,agl);
                                        ...
                                        Point& centerPt)
{
    ...
    Point extpt(x,y);
#endif
    bool iscw = isCpRightPos == true ? true : false;
                                                ...
                                                vector<Point>&vPoint)
{
    auto HexPt = [](double perHei,double hisAng,double R,double curAngle,
        ...
        double _x = R * std::cos(curAngle * M_PI / 180);
        double _y = R * std::sin(curAngle * M_PI / 180);
        double x = cp.X() + _x;
        double y = cp.Y() + _y;
