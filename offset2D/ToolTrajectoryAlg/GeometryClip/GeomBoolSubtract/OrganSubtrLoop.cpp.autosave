                                                ...
                                                vector<DefElem>& vElement)
{
    ...
    tempLoop.m_vElem[eleIndex]._label._hasMerge = true;
    Point preEp,curSp;
    numcal.ElementEndpoint(startEle,true,preEp);
    if(subsol.IsTerminateSearch(endpoint,preEp,vElement)){
        return;
    }
    int i = numcal.NexId(tempLoop.Size(),eleIndex);
    while (i < tempLoop.Size()) {/**
        if(i == 5){
            int tem = 1;
        }*/
        ...
        if(gocal.IsIdenticalPoint2D(preEp,curSp,PreErr5_8)){
            ...
            tempLoop.m_vElem[i]._label._hasMerge = true;
            numcal.ElementEndpoint(ele,true,preEp);
            if(subsol.IsTerminateSearch(endpoint,preEp,vElement))
                return;
            i = numcal.NexId(tempLoop.Size(),i);
            continue;
        }
        if(subsol.IsTerminateSearch(endpoint,preEp,vElement))
            return;
                                             ...
                                             vector<DefElem> &vElement)
{
    ...
    int i = numcal.PreId(loop.Size(),eleIndex);
    while (i < loop.Size()) {
