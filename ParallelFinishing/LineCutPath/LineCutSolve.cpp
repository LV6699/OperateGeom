...
#include"../../offset2D/ToolTrajectoryAlg/GeometryCalculate/RelativeContain/LimitCoord.h"
#include"../../offset2D/ToolTrajectoryAlg/OperCurve2D/NumeCalculate/ElemCalculate.h"
...
#endif
void LineCutSolve::SetLoopClose(ofts::DefLoop& loop)
{
    ElemCalculate elecal;
    for (int i = 0; i < loop.Size()-1; ++i) {
        DefElem& pre = loop.m_vElem[i];
        DefElem& nex = loop.m_vElem[i+1];
        ofts::Point s = elecal.ElementEndpoint(nex,false);
        elecal.SetElementEndpoint(s,true,pre);
    }
    DefElem fir = loop.First();
    DefElem las = loop.Last();
    ofts::Point sp = elecal.ElementEndpoint(fir,false);
    elecal.SetElementEndpoint(sp,true,las);
    loop.SetLast(las);
}
double LineCutSolve::LoopMaxZCoord(const ofts::DefLoop& loop){
    ElemCalculate elecal;
    double z = Min_Value;
    for(auto& d : loop.m_vElem){
        ofts::Point sp = elecal.ElementEndpoint(d,false),
...
void LineCutSolve::ResetLoopOrderBaseX(ofts::DefLoop& loop)
{
    int id = 0;
    double x = loop.IndexElem(0).GetSeg().GetSp().X();
    for (int i = 0; i < loop.Size(); ++i) {
        if(loop.IndexElem(i).GetSeg().GetSp().X() < x){
            x = loop.IndexElem(i).GetSeg().GetSp().X();
            id = i;
        }
    }    ofts::DefLoop res = loop;
    res.Clear();
    for (int i = id; i < loop.Size(); ++i) {
        res.AddElem(loop.IndexElem(i));
    }
    for (int i = 0; i < id; ++i) {
        res.AddElem(loop.IndexElem(i));
    }
                                    ...
                                    ofts::Point& p2)
{
    ...
    _mainwind->myOccView->myView->FitAll(0.7);
#endif
    ofts::Point minp;
    ///limco.LoopLimitCoordPoint(loop,true,false,minp);
    minp = LimitCoord2D::LoopLimitPoint(loop,true,false);
    double len = 4000,radians = agl * M_PI / 180.0;
    double dx = std::cos(radians),dy = std::sin(radians);
    ///浠庡乏涓婅寮€濮,闇€瑕乤gl 灏忎簬180.
    ofts::Point ext1(minp.X() - dx * len, minp.Y() - dy * len);
    ofts::Point ext2(minp.X() + dx * len, minp.Y() + dy * len);
    if(agl < 90){
        if(ext1.X() > ext2.X()){
            std::swap(ext1,ext2);
        }
    ...
    while (true) {
        ...
        ofts::Point p1_,p2_;
        equ.EquidistantSegment(ext1,ext2,oftDist,true,p1_,p2_);
                                     ...
                                     double err, vector<IntPnt> &intPts)
{
    ...
    for (int i = 0; i < intPts.size(); ++i) {
        ...
        const DefElem& ele_ = loops[intPts[i+1]._lId].IndexElem(intPts[i+1]._eId);
    ...
    to_remove.resize(intPts.size(), false);
    for (size_t i = 0; i < intPts.size() - 1; ++i) {
        if (removed_previous) {
        ...
        if (intPts[i]._p.Distance2D(intPts[i+1]._p) < err) {
            to_remove[i] = true;
            to_remove[i+1] = true;
            removed_previous = true; ///璺宠繃涓嬩竴涓偣(閬垮厤閲嶅澶勭悊)
        }
    }
    intPts.erase(std::remove_if(intPts.begin(),intPts.end(),
                                [&to_remove, &intPts](const IntPnt& p) {
                                           ...
                                           vector<IntPnt>& intPts)
{
    ...
    for (int i = 0; i < intPts.size(); ++i) {
        int p = elecal.PreId(intPts.size(),i),
                n = elecal.NexId(intPts.size(),i);
        intPts[i].pre = &intPts[p];
        intPts[i].nex = &intPts[n];
    }
#if 0
    _mainwind->myOccView->getContext()->RemoveAll(true);
    DisplayGeom().DisplayLoop(loop,_colors[8],1,true);
    _mainwind->myOccView->fitAll();
    for (int i = 0; i < intPts.size(); ++i) {
        DisplayGeom().DisplayLoop(loop,_colors[8],1,true);
        DisplayGeom().DisplayPoint(intPts[i].P(),_colors[2],true,i);
        DisplayGeom().DisplayPoint(intPts[i].pre->P(),_colors[1]);
        DisplayGeom().DisplayPoint(intPts[i].nex->P(),_colors[1]);
        _mainwind->myOccView->getContext()->RemoveAll(true);
    }
    int tem = 1;
#endif
