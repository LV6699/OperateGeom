                                       ...
                                       double& angle)
{
    ...
    /// 处理特殊情况：如果水平长度为0，说明线段垂直于xy 平面，夹角为90 度
    if (horLen < PreErr_10) {
        angle = 90;
        return;
    }
                                       ...
                                       bool& isPara)
{
    ...
    CalLineParameters(l1Sp,l1Ep,para1);
    CalLineParameters(l2Sp,l2Ep,para2);
...
{
    double R1 = std::sqrt(std::pow(arc.GetSp().X() -
                                   arc.GetCp().X(), 2) +
                          std::pow(arc.GetSp().Y() -
                                   arc.GetCp().Y(), 2)),
            R2 = std::sqrt(std::pow(arc.GetEp().X() -
                                    arc.GetCp().X(), 2) +
                           std::pow(arc.GetEp().Y() -
                                    arc.GetCp().Y(), 2));
    return (R1 + R2) / 2;
...
{
    return TwoPointDistance(seg.GetSp(),seg.GetEp());
}
bool GeomCalculate::IsPntDisGreaterVal(const Point& P1,
                                       const Point& P2,
                                       double val)
{
    double dis = std::sqrt(std::pow(P2.X() - P1.X(),2) +
                           std::pow(P2.Y() - P1.Y(),2));
    if(dis >= val)
        return true;
    return false;
                                          ...
                                          double& angle)
{
    ...
    double centerAgl;
    TwoAngleIncludedAngle(startAgl,endAgl,arc._isCW,centerAgl);
                                          ...
                                          vector<Point>& points)
{
    ...
    /// 预先计算倒数后,可以用乘法代替后续所有除法操作.
    double invTotalLength = 1 / len;
                                          ...
                                          vector<Point>& vPoint)
{
    ...
    DefArc arc_(sp,ep,cp,R,iscw),arc__(ep,sp,cp,R,iscw);
                                         ...
                                         vector<Point>&vPoint)
{
    ...
    case LINETYPE:{
        ...
        DiscreteLineToPoint(ele.GetSeg().GetSp(),
                            ele.GetSeg().GetEp(),
                            segStep,vPoint);
        break;
    }
    case ARCTYPE:{
        ...
        double step = arcStep;
        if(!isStepAngle){
            step = matcal.AngleBaseBowHeight(arc.R(),arcStep);
        }
    ...
    case CIRCLETYPE:{
        ...
        DefCircle circle = ele.GetCircle();
        double step = arcStep;
        if(!isStepAngle){
            step = matcal.AngleBaseBowHeight(circle.R(),arcStep);
        }
        DiscreteCircleToPoint(circle.GetCp(),
                              circle.R(),
                              true,step,vPoint);
        break;
    }
                                          ...
                                          bool& isSameDir)
{
    ...
    TwoPointDistance(origArcSp,arcEp,origSp_ostEp_dis);
    TwoPointDistance(origArcEp,arcSp,origEp_ostSp_dis);
                                 ...
                                 bool& isInFan)
{
    ...
    ArcEndpointAngle(arcData._arcSp,arcData._arcEp,
                     arcData._arcCp,arcData._isCW,
                     startAgl,endAgl);
    double theta = atan2((objPnt._y - arcData._arcCp._y),
                         (objPnt._x - arcData._arcCp._x))
            * 180.0 / M_PI;
    theta = (theta < 0) ? (theta += 360) : theta;
    AngleIsBetweenAngles(startAgl,endAgl,arcData._isCW,
...
double GeomCalculate::AngleMidAngle(double sAng,double eAng,bool rotDir)
{
    double absAng;
                                ...
                                Point& midPnt)
{
    ...
    ArcEndpointAngle(arc._arcSp,arc._arcEp,arc._arcCp,
                     arc._isCW,sAng,eAng);
    TwoAngleIncludedAngle(sAng,eAng,arc._isCW,absAng);
    double R = arc.R();
    if((R <= PreErr5_2) || !MathCalculate().IsLegalValue(R)){
        DefArc arc_ = arc;
        ElemCalculate ().InitialArcRadius(arc_);
        R = arc_.R();
    }
...
{
    bool iscw,calRot;
    TriangleRotDirection(l1Sp,l1Ep,l2Ep,iscw);
    calRot = iscw == true ? false : true;
                                         ...
                                         Point& P)
{
    ...
    if(isCloseSp){
        if(tup.IsHorizontalLine(lSp,lEp,Base_LineHor_Err)){
            double startX = lSp.X();
            if(startX < lEp.X()){startX = startX + stepVal;}
            else{startX = startX - stepVal;}
            P.SetX(startX);
            P.SetY((lSp.Y() + lEp.Y()) / 2);
        }else if(tup.IsVerticalLine(lSp,lEp,Base_LineVer_Err)){
            double startY = lSp.Y();
            if(startY < lEp.Y()){startY = startY + stepVal;}
            else{startY = startY - stepVal;}
        ...
        if(tup.IsHorizontalLine(lSp,lEp,Base_LineHor_Err)){
            double endX = lEp.X();
            if(endX < lSp.X()){endX = endX + stepVal;}
            else{endX = endX - stepVal;}
            P.SetX(endX);
            P.SetY((lSp.Y() + lEp.Y()) / 2);
        }
        else if(tup.IsVerticalLine(lSp,lEp,Base_LineVer_Err)){
            double endY = lEp.Y();
            if(endY < lSp.Y()){endY = endY + stepVal;}
            else{endY = endY - stepVal;}
            P.SetX((lSp.X() + lEp.X()) / 2);
            P.SetY(endY);
        }
        else{
...
void GeomCalculate::ArcEndpointAngle(const Point &sP,const Point &eP,
                                     const Point &cP,const bool &iscw,
                                     double& startAgl,
                                     double& endAgl)
{
...
void GeomCalculate::ArcEndpointAngle(const DefArc& arc,
                                     ...
                                     double& endAgl)
{
    ArcEndpointAngle(arc.GetSp(),arc.GetEp(),
                     arc.GetCp(),arc.IsCW(),
                     startAgl,endAgl);
}
                                   ...
                                   double& angle)
{
    ...
    ArcEndpointAngle(arc._arcSp,arc._arcEp,
                     ...
                     startAgl,endAgl);
    TwoAngleIncludedAngle(startAgl,endAgl,arc._isCW,angle);
}
                                       ...
                                       double& len)
{
    ...
    if(arc.GetSp().IsSamePoint2D(arc.GetEp(),PreErr5_16)){
        len = 0;
        return;
    }
                                         ...
                                         Point &P)
{
    ...
    switch (ele._type) {
    case LINETYPE:
        CloseSegmentEndpoint(ele.GetSeg().GetSp(),
                             ele.GetSeg().GetEp(),
                             stepVal,!isCloseEp,P);
        break;
    case ARCTYPE:
        golg.CloseArcEndpointBaseAngle(ele.GetArc(),
                                       isCloseEp,stepVal,P);
        break;
    default:
        break;
...
{
    DefArc arc_ = arc;
    arc_.OptInitR(PreErr5_2);
    double R = arc_.R();
    Point mp = arc._arcSp.Midpoint(arc._arcEp);
    double dis = arc._arcCp.Distance2D(mp);
    double angle;
...
{
    Point P_;
    PointAroundFixedRotate(P,arc.GetCp(),90,arc.IsCW(),P_);
    double a;
    SegmentAngle(arc.GetCp(),P_,a);
    return a;
}
bool GeomCalculate::ThreePointCollinear(const Point& p0,
                                        ...
                                        double err)
{
    Point v1 = p1 - p0;  // 向量 p0 → p1
    Point v2 = p2 - p0;  // 向量 p0 → p2
...
void GeomCalculate::SimplePointsBaseLine(double err, vector<Point> &points)
{
    ...
    for (size_t i = 2; i < n; ++i) {
        ...
        if (!ThreePointCollinear(points[s],points[i-1],points[i],err)) {
            /// 如果不能共线,保存当前线段
            if (s != i-1 && !points[s].IsSamePoint3D(points[i-1],err)) {
                compressed.emplace_back(points[i-1]);
            }
            s = i-1;
        }
    }
    ...
    while (left + 2 < points.size()) {
        ...
        for (size_t i = left + 1; i < right; ++i) {
            if (!ThreePointCollinear(points[left], points[i], points[right], err)) {
                allCollinear = false;
                break;
            }
        }
