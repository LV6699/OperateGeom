                                               ...
                                               shared_ptr<OffsetNode>& pathNode)
{
    ...
    double firOftVal = param._offsetUnit.FirstOftVal(),
    ...
    while(!regions.empty()){
        if(!regions[0].IsValidMatch()){
            regions.erase(regions.begin());continue;
        }
        GeomArea& region = regions[0];
        DefLoop bloop = region._bndLoop;
        vector<DefLoop>iloops = region._vIslLoop;
        if(isFixOft && bloop.PastOftNum() >= fixOftNum){break;}
        shared_ptr<OffsetNode>bNode = make_shared<OffsetNode>();
        if(param._pathTech.IsAddOriginLoop() && bloop.PastOftNum() == 0){
            bNode->AddLoop(region._obLoop);
        }
