...
#endif
using namespace std;
void HorArcSteer::HorizontalArcSteer(ofts::DefLoop& loop,
                                     ofts::MillDirction millDir,
                                     bool isEntry,double R,double agl,
                                     ofts::DefArc& arc)
{
#if 0
    ofts::Point pt(98.22983491420746,97.882060110569,5);
    ...
    ofts::Point sp,cp,ep;
    ofts::DefElem elem = loop.FirstElem();
    ...
    arc = ofts::DefArc(sp,ep,cp,R,arcRot);
    if(isEntry){
        arc.Reverse();
    }
}