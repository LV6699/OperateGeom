                                                    ...
                                                    Point &center)
{
    ...
    while (!isExiCenter && radius > SmoothValLimit) {
        ...
        equi.EquidistantSegment(l1Sp,l1Ep,radius,false,outL1Sp,outL1Ep);
        equi.EquidistantSegment(l2Sp,l2Ep,radius,false,outL2Sp,outL2Ep);
        GeomIntRes intRes;
        ...
        if(intRes._isExistInt && intRes._isExiGeoInt){
            ...
            equi.EquidistantSegment(l1Sp,l1Ep,radius,true,witL1Sp,witL1Ep);
            equi.EquidistantSegment(l2Sp,l2Ep,radius,true,witL2Sp,witL2Ep);
            GeomIntRes intRes1;
                                                ...
                                                Point &center)
{
    ...
    while (!isExiCenter && radius > SmoothValLimit) {
        Point out_sp,out_ep;
        DefArc out_arc;
        equi.EquidistantSegment(lSp,lEp,radius,false,
                                out_sp,out_ep);
        bool isExiEquiArc = false;
        equi.EquidistantArc(arc,radius,false,
                            false,isExiEquiArc,
                            out_arc);
        if(!isExiEquiArc){
            radius = radius / SmoothTimes;
        ...
        if(intRes._isExistInt){
            ...
            Point wit_sp,wit_ep;
            DefArc wit_arc;
            equi.EquidistantSegment(lSp,lEp,radius,true,
                                    wit_sp,wit_ep);
            bool isExiEquiArc1 = false;
            equi.EquidistantArc(arc,radius,true,
                                false,isExiEquiArc1,
                                wit_arc);
            if(!isExiEquiArc1){
                radius = radius / SmoothTimes;
                                            ...
                                            Point &center)
{
    ...
    while (!isExiCenter && radius > SmoothValLimit){
        ...
        equi.EquidistantArc(arc1,radius,false,
                            false,isExiOutEquArc1,out_arc1);
        equi.EquidistantArc(arc2,radius,false,
        ...
        if(intRes._isExistInt){
            ...
            equi.EquidistantArc(arc1,radius,true,false,
                                isExiWitEquArc1,wit_arc1);
            equi.EquidistantArc(arc2,radius,true,false,
            ...
            if(intRes._intNum == 1){
                ...
                eleint.GetCloseArcExtremePnt(wit_arc1,intRes._intPnt1,
                                             intRes._intPnt2,false,center);
            }
            isExiCenter = true;
        }
