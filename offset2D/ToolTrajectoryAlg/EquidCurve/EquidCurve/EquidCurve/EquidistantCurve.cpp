                                      ...
                                      DefArc& equArc)
{
    ...
    double sym = leftward == false ? 1 : -1;
    Point SP =oriArc.GetSp();
    Point EP =oriArc.GetEp();
    Point CP =oriArc.GetCp();
    double R = oriArc.R();
    if(isSetRadius){
    ...
    double R_ = oriArc._isCW ? R - oftD * sym : R + oftD * sym;
...
void EquidistantCurve::EquidistantSegment(const Point& orilSp,
                                          ...
                                          Point& lSp, Point& lEp)
{
    //double sym = prof == OUTSIDEPROFILE ? 1 : -1;
    double sym = leftward == false ? 1 : -1;
    double spx = orilSp.X();
    double spy = orilSp.Y();
    double epx = orilEp.X();
    ...
    if (A == 0 && B != 0) {
        ...
        lSp.SetX(orilSp.X() + oftD * a);
        lSp.SetY(orilSp.Y() + oftD * b);
        lEp.SetX(orilEp.X() + oftD * a);
        lEp.SetY(orilEp.Y() + oftD * b);
#else
        double vaule = sym * oftD;
...
void EquidistantCurve::EquidistantCircle(const Point& cp,double radius,
                                         ...
                                         DefCircle& circle)
{
    //double R = proType == OUTSIDEPROFILE ? (radius - oftD) : (radius + oftD);
    double R = leftward == false ? (radius - oftD) : (radius + oftD);
...
bool EquidistantCurve::IsExistEquElement(const DefElem& ele,
                                         ...
                                         double value)
{
    if(/*proType == WITHINPROFILE*/leftward || ele._type == LINETYPE){
        return true;
    }
    if(ele._type == ARCTYPE){
        DefArc arc = ele._arc;
                                          ...
                                          DefElem& equEle)
{
    if(ele._type == LINETYPE){
        ...
        equEle._type = LINETYPE;
        isExiEquiEle = true;
    }
    else if(ele._type == ARCTYPE){
        EquidistantArc(ele._arc,oftD,leftward,true,
                       isExiEquiEle,equEle._arc);
        equEle._type = ARCTYPE;
    }
    else if(ele._type == CIRCLETYPE){
        DefCircle circle;
        EquidistantCircle(ele._circle.GetCp(),
                          ele._circle.R(),
                          leftward,oftD,circle);
        equEle.SetCircle(circle);
    }
}
                                              ...
                                              DefArc &arc)
{
    ...
    double endAgl = std::atan2((arcData._arcEp.Y() -
                               ...
                               (arcData._arcEp.X() -
                                arcData._arcCp.X()));
    arc._arcSp._x = arc._arcCp.X() + R * std::cos(startAgl);
    arc._arcSp._y = arc._arcCp.Y() + R * std::sin(startAgl);
