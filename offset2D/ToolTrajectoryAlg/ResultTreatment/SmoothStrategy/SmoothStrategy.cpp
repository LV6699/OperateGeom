                                            ...
                                            vector<DefLoop>&vLoop)
{
    ...
    for (int lo = 0; lo < vArrLoop.size(); ++lo) {/**
        ...
        if(vArrLoop[lo].PastOftNum() > limOftNum){
        ...
        for (int i = 0; i < loop.Size(); ++i) {/**
            ...
            if(loop.IndexElem(i).Type() == LINETYPE){
                ...
                if(loop.m_vElem[nex]._type == LINETYPE){
                ...
                else if(loop.m_vElem[nex]._type == ARCTYPE){
                    double angle;
                    DefArc arc = loop.m_vElem[nex]._arc;
            ...
            else if(loop.IndexElem(i).Type() == ARCTYPE){
                                                   ...
                                                   DefArc& arc)
{
    ...
    gocal.CalAngleBetweenTwoVectors(l1Sp,l1Ep,l2Sp,l2Ep,sTheta,eTheta,aveAgl);
    gocal.TriangleRotDirection(l1Sp,l1Ep,l2Ep,arc._isCW);
    bool iscw = !arc.IsCW();
    double aglAbsVal;
    gocal.TwoAngleIncludedAngle(sTheta,aveAgl,iscw,aglAbsVal);
    aglAbsVal = aglAbsVal * M_PI / 180.0;
    double calAveAgl = aveAgl * M_PI / 180.0;
    double xInc,yInc;
    if((abs(aveAgl - 270.0) > PreErr_6) &&
            (abs(aveAgl - 90.0) > PreErr_6)){
        //double slope = std::tan(calAveAgl);
                                             ...
                                             bool& iscw)
{
    ...
    if(MathCalculate().IsNumericEqual(slop1,slop2,PreErr5_12)){
        ...
        gocal.ArcMidpoint(arc1,arcMidPnt);
        gocal.TriangleRotDirection(arcMidPnt,arc1._arcEp,arc2._arcEp,iscw);
    }
    else{
        gocal.TriangleRotDirection(arc1._arcSp,arc1._arcEp,arc2._arcEp,iscw);
    }
}
