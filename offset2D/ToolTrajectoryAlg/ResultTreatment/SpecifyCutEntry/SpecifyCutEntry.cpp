                                              ...
                                              bool isToOutside)
{
    ...
    SpecifyCutEntryPoint(rootNode->_loops,isToOutside);
    for (int i = 0; i < rootNode->_nodes.size(); ++i) {
        RedesignPathBaseOnEntry(rootNode->_nodes[i],isToOutside);
    }
}
                                             ...
                                             vector<DefLoop>&vLoop)
{
    ...
    for (int i = 0; i < vLoop.size() - 1; ++i) {
        Point closePt;
        int nex = numcal.NexId(vLoop.size(),i),index;
