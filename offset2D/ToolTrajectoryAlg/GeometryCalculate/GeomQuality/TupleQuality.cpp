                                          ...
                                          double pre)
{
    ...
    if(go.IsIdenticalPoint(l1Sp,l2Sp,pre) &&
        ...
        return true;
    if(go.IsIdenticalPoint(l1Sp,l2Ep,pre) &&
            go.IsIdenticalPoint(l1Ep,l2Sp,pre))
        return true;
    return false;
}
                                         ...
                                         double pre)
{
    ...
    if(go.IsIdenticalPoint(arc1._arcSp,arc2._arcSp,pre) &&
            ...
            arc1.IsCW() == arc2.IsCW())
        return true;
    if(go.IsIdenticalPoint(arc1._arcSp,arc2._arcEp,pre) &&
            go.IsIdenticalPoint(arc1._arcEp,arc2._arcSp,pre) &&
            arc1.IsCW() != arc2.IsCW())
        return true;
    return false;
}
...
bool TupleQuality::IsEleLocatedOnEle(const DefElem& refEle,
                                     ...
                                     double pre)
{
    if(refEle._type != ele._type)
        return false;
    if(refEle._type == LINETYPE){
    ...
    else if(refEle._type == ARCTYPE){
        return IsArcLocatedOnArc(refEle._arc,ele._arc,pre);
    }
    else if(refEle._type == CIRCLETYPE){
...
{
    ///lSp 表示切线起点.
    if(ele._type != LINETYPE && ele._type != ARCTYPE)
        return;
    GeomCalculate gocal;
    ...
    gocal.PointAroundFixedRotate(cp,basePt,A,iscw,rotResPt);
    lSp = basePt;
    lEp = rotResPt;
}
