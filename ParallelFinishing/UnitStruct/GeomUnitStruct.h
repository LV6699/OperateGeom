...
namespace OftStr = OffsetStruct;
namespace GeomStruct {
class PntSet
{
public:
    PntSet() {}
    PntSet(const std::vector<OffsetStruct::Point>&v) : _vPnt(v) {}
public:
    std::vector<OffsetStruct::Point>_vPnt;
};
}
namespace GridArea {
class LinePnt{
public:
    LinePnt(){}
    LinePnt(const std::vector<OffsetStruct::Point> &v){_vPnt = v;}
    void PushBack(const OffsetStruct::Point &v){_vPnt.push_back(v);}
    OffsetStruct::Point IndexPt(int i) const{return _vPnt[i];}
    OffsetStruct::Point Back() const{return _vPnt.back();}
    OffsetStruct::Point Front() const{return _vPnt.front();}
    OffsetStruct::Point * IndexNormal(int i) const{return _vPnt[i]._lable._normal;}
    OffsetStruct::Point TailSecond() const{
...
public:
    size_t operator()(const DefEdge& e) const {
        return std::hash<int>()(e._a) ^ (std::hash<int>()(e._b) << 1);
    }
};
class TriIndex{
public:
...
public:
    ...
    bool _isBorder0 = false,_isBorder1 = false,_isBorder2 = false;
    bool _isE0Border = false,_isE1Border = false,_isE2Border = false;
};
class TriInfo{
public:
    struct AdjPoint{
...
public:
    ...
    void CalculateAngel(){
        OffsetStruct::Point ab = _p0 - _p1,ac = _p0 - _p2,
                _normal = ab.Cross(ac);
    ...
    OftStr::Point P0() const{return _p0;}
    OftStr::Point P1() const{return _p1;}
...
public:
    ...
    TriLable _lable;
    TriBorder _border;
    OffsetStruct::Point _normal;
    std::vector<int>_adjs[3];
};
class Triangles{
public:
    Triangles(){}
    Triangles(const std::vector<OftStr::Point>&v,
...
public:
    ...
    double _horY,verX;
    std::vector<OffsetStruct::DefArc> _vHorArc,_vVerArc;
};
class RecAglBor{
public:
    RecAglBor(){}
public:
    ...
    std::vector<OffsetStruct::Point>_vBordPt;
};
class RecLable{
public:
    RecLable(){}
...
public:
    ...
    GridArea::LinePnt* _seg[4];///鎸囬拡鏁�?�?.
    OffsetStruct::Point _p0,_p1,_p2,_p3;
    OffsetStruct::Point _mp0,_mp1,_mp2,_mp3,_cp;
    OffsetStruct::Point _maxZPt,_minZPt;
};
/**
p1------p2    ----l2---->
 |      |    |          |
...
public:
    void SetVertex(){
        ...
        _vertex._mp0 = EdgeMidpoint(0);
        _vertex._mp1 = EdgeMidpoint(1);
        _vertex._mp2 = EdgeMidpoint(2);
        _vertex._mp3 = EdgeMidpoint(3);
        _vertex._cp._x = (_vertex._p0._x + _vertex._p1._x + _vertex._p2._x + _vertex._p3._x) / 4;
        _vertex._cp._y = (_vertex._p0._y + _vertex._p1._y + _vertex._p2._y + _vertex._p3._y) / 4;
        _vertex._cp._z = (_vertex._p0._z + _vertex._p1._z + _vertex._p2._z + _vertex._p3._z) / 4;
    }
    ...
    void SetExtremeZPoint() {
        _vertex._maxZPt = _vertex._p0.Z() > _vertex._p1.Z() ? _vertex._p0 : _vertex._p1,
                _vertex._minZPt = _vertex._p0.Z() < _vertex._p1.Z() ? _vertex._p0 : _vertex._p1;
    ...
    void ReverseSegment(int i){_vertex._vSeg[i].Reverse();}
    bool IsSteep() const{return _isSteep;}
...
public:
    ...
    vector<vector<OffsetStruct::Point>> GetPlanePoint() const{return _vArrPoint;}
    int PlanePointNum() const{return _pntNum;}
public:
    int _pntNum = 0;
    vector<vector<OffsetStruct::Point>>_vArrPoint;
};
class ModelStruct{
public:
    ModelStruct(){}
    ...
    void SetVertical(const std::vector<std::vector<GridArea::LinePnt>>&v){
        _vVerLine = v;}
    void SetRectangle(const std::vector<std::vector<GeomUnitStruct::Rectangle>>&r){
        _vArrRec = r;}
    void IniTriIndex(){
...
public:
     IntPnt(){}
     IntPnt(int e,bool isEndpt,const OftStr::Point& p) :
        _eId(e),_isEndpt(isEndpt),_p(p){}
...
public:
    int _id = -1;              ///浜ょ偣绱㈠紩
    int _eId = -1;             ///浜ょ偣鎵�?鍦ㄦ洸绾�?厓绱犵储�?
    int _lId = -1;             ///浜ょ偣鎵�?鍦ㄧ幆绱㈠紩
    int _lineId = -1;          ///浜ょ偣鎵�?鍦ㄦ壂鎺犵嚎绱㈠�?
    bool _isEndpt = false;     ///浜ょ偣鎵�?鍦ㄦ洸绾�?厓绱犵储�?
    bool _hasVisit = false;    ///浜ょ偣鏄惁�?�茬粡璁块棶
    OftStr::Point _p;          ///浜ょ�?
    IntPnt *pre = nullptr;     ///浜ょ偣鎵�?鍦ㄦ洸绾跨浉瀵�?�浜ょ偣鐨勫墠涓�?涓氦�?
    IntPnt *nex = nullptr;     ///浜ょ偣鎵�?鍦ㄦ洸绾跨浉瀵�?�浜ょ偣鐨勫悗涓�?涓氦�?
    IntPnt *cnt = nullptr;     ///涓庝氦鐐瑰湪鍚屼竴鎵�?绾夸笂鐨�?繛鎺ョ偣
};
class IntLine{
public:
    IntLine(){}
