                                           ...
                                           shared_ptr<OffsetNode>& symNode)
{
    ...
    ///rootNode->_nodes.push_back(symNode);
}
                                            ...
                                            vector<DefLoop>& vLoop)
{
    ...
    for (int lo = 0; lo < rootNode->_loops.size(); ++lo) {
        if(rootNode->_loops[lo].m_vElem.empty()){
            continue;
        }
        DefLoop loop;
        DefSeg seg1,seg2;
        bool isExiSym = false;
        DefElem ele = rootNode->_loops[lo].m_vElem[0];
        double z = ElemCalculate().LoopZCoordinate(rootNode->_loops[lo]);
    ...
    for (int i = 0; i < rootNode->_nodes.size(); ++i) {
        NodePathDirectionSymbol(toolRadius,rootNode->_nodes[i],vLoop);
    }
}
                                           ...
                                           DefSeg& segment2)
{
    ...
    numcal.PointSplitElement(ele,midPt,true,preEle);
    numcal.PointSplitElement(ele,midPt,false,nexEle);
    bool isCloseSp = false;
    double stepVal = toolRadius / 2.5;
    if(ele.Type() == ARCTYPE){
        stepVal = matcal.ArcLenthToAngle(stepVal,preEle.
                                         _arc.R());
    }
    ///numcal.ElementEndpointStepVal(ele,stepVal,stepVal);;
    golg.ApproachCurveEndpoint(preEle,stepVal,true,isCloseSp,movePt);
    ///movePt 和midPt 组成新的线段,线段起点是movePt,线段终点是midPt.
    Point P1,P2;
    double angle = 35;
    ///double len = dis / std::cos(angle * M_PI / 180);
    gocal.PointAroundFixedRotate(movePt,midPt,angle,true,P1);
    gocal.PointAroundFixedRotate(movePt,midPt,angle,false,P2);
    segment1.SetSp(midPt);
    segment1.SetEp(P1);
    segment2.SetSp(midPt);
    segment2.SetEp(P2);
    isExiSym = true;
}
