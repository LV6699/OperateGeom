...
void GeomTangency::IsLineCircleTan(const Point& lSp,
                                   const Point& lEp,
                                   const Point& cp,
                                   double R, double pre,
                                   bool& isTan)
...
bool GeomTangency::IsLineCircleTan(const Point& lSp,
                                   const Point& lEp,
                                   const Point& cp,
                                   double R, double pre)
{
    bool isTan = false;
    IsLineCircleTan(lSp,lEp,cp,R,pre,isTan);
    return isTan;
}
...
bool GeomTangency::IsEleEleTangency(const DefElem& ele1,
                                    ...
                                    double pre)
{
    if(!isJudLine && ele1._type == LINETYPE && ele2._type == LINETYPE){
        return false;
    }
    ...
    bool isTan = false;
    if(isLineCir){
        IsLineCircleTan(lSp,lEp,cp1,R1,pre,isTan);
    }
    else{
