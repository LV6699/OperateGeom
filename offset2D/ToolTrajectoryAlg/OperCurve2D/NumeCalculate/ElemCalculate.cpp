...
{
    resLoop = DefLoop();
    resLoop.SetCurves(oriLoop.LoopElem());
}
                                           ...
                                           DefLoop &loop)
{
    ...
    loop_.m_vElem.clear();
    for (int i = 0; i < oriLoop.Size(); ++i) {
        DefElem ele;
        if(oriLoop.IndexElem(i).Type() == LINETYPE)
            ele._segment = oriLoop.IndexElem(i)._segment;
        else if(oriLoop.IndexElem(i).Type() == ARCTYPE)
            ele._arc = oriLoop.IndexElem(i)._arc;
        else if(oriLoop.IndexElem(i).Type() == CIRCLETYPE)
            ele._circle = oriLoop.IndexElem(i)._circle;
        ele.SetType(oriLoop.IndexElem(i).Type());
        loop_.AddElem(ele);
    }
    if(isResetLoop)
                                           ...
                                           bool isSetLoop)
{
    for (int i = 0; i < loop.m_vElem.size(); ++i) {
        ...
        if(loop.m_vElem[i]._type == LINETYPE)
            ele.SetSeg(loop.IndexElem(i).GetSeg());
        else if(loop.m_vElem[i]._type == ARCTYPE)
            ele.SetArc(loop.IndexElem(i).GetArc());
        else if(loop.m_vElem[i]._type == CIRCLETYPE)
            ele.SetCircle(loop.IndexElem(i).GetCircle());
        ele.SetType(loop.IndexElem(i).Type());
        loop.SetIdElem(ele,i);
    }
    if(isSetLoop){
...
double ElemCalculate::LoopZCoordinate(const DefLoop& loop)
{
    if(loop.IsEmpty())
        return 0;
    if(loop.IndexElem(0).Type() == LINETYPE)
        return loop.IndexElem(0).GetSeg().GetSp().Z();
    else if(loop.IndexElem(0).Type() == ARCTYPE)
        return loop.IndexElem(0).GetArc().GetCp().Z();
    else if(loop.IndexElem(0).Type() == CIRCLETYPE)
        return loop.IndexElem(0).GetCircle().GetCp().Z();
    return 0;
}
...
void ElemCalculate::DeleIllegalCurveNumLoop(vector<DefLoop>&vLoop)
{
    size_t i = 0;
    while (i < vLoop.size()) {
        bool isLegal = false;
        IsLegalCurveNumLoop(vLoop[i],isLegal);
        if(isLegal){
            i ++;continue;
        }
        vLoop.erase(vLoop.begin() + i);
                                   ...
                                   int& num)
{
    ...
    for(int i = 0;i < rootNode->_nodes.size();i++){
        GetNodeLoopNum(rootNode->_nodes[i],num);
    }
}
                                  ...
                                  int& num)
{
    ...
    for(int i = 0;i < rootNode->_loops.size();i++){
        num += rootNode->_loops[i].m_vElem.size();
    }
    for(int i = 0;i < rootNode->_nodes.size();i++){
        GetNodeEleNum(rootNode->_nodes[i],num);
    }
}
                                    ...
                                    double& lenth)
{
    ...
    for(int i = 0;i < rootNode->_loops.size();i++){
        double len = 0;
        LoopGeometryLenth(rootNode->_loops[i],len);
        lenth += len;
    }
    for(int i = 0;i < rootNode->_nodes.size();i++){
        GetNodeEleLenth(rootNode->_nodes[i],lenth);
    }
}
bool ElemCalculate::IsValidPoint(const Point& P,bool is2D)
{
    ...
    if (is2D) {
        ...
        if (!mat.IsLegalValue(P._x) || !mat.IsLegalValue(P._y) || !mat.IsLegalValue(P.Z()))
            return false;
    }
    return true;
}
                                        ...
                                        bool& isLegNum)
{
    ...
    if(loop.m_vElem.size() > 2 ||
            (!loop.m_vElem.empty() && loop.IsOpen()))
        return;
                                       ...
                                       double pre)
{
    ...
    if(ele1._type == LINETYPE){
        ...
        if(ele1._arc._isCW != ele2._arc._isCW)
            return false;
...
void ElemCalculate::DeleteIndexLoop(const vector<size_t>& vIndex,
                                    vector<DefLoop>& vLoop)
{
...
void ElemCalculate::DeleteIndexElement(const vector<size_t>&vIndex,
                                    ...
                                    bool isEp,Point& P)
{
    ...
    case LINETYPE:{
        if(isEp)
            P = ele._segment.GetEp();
        else
            P = ele._segment.GetSp();
        break;
    }
    case ARCTYPE:{
        if(isEp)
    ...
    case CIRCLETYPE:{
        P.SetX(ele._circle.GetCp().X() +
               ele._circle.R());
        P.SetY(ele._circle.GetCp().Y());
        P.SetZ(ele._circle.GetCp().Z());
        break;
    }
    default:
        break;
...
Point ElemCalculate::ElementEndpoint(const DefElem &ele,bool isEp)
{
    ...
    case LINETYPE:{
        if(isEp){
            return ele._segment.GetEp();
        }
        else{
            return ele._segment.GetSp();
        }
    }
    case ARCTYPE:{
        if(isEp){
            return ele._arc.GetEp();
        }
        else{
            return ele._arc.GetSp();
        }
    }
    case CIRCLETYPE:{
        return ele._circle.GetCp();
    }
    default:
        break;
    }
...
{
    if(startIndex < 0 || startIndex > loop.Size() - 1)
        return;
    if(!isOrder){
                                      ...
                                      DefElem& newEle)
{
    ...
    if(ele._type == LINETYPE){
        DefSeg seg;
        if(isChEndPnt){
            seg = DefSeg(ele._segment.GetSp(),P);
        }
        else{
        ...
        DefArc arc;
        if(isChEndPnt){
            arc = DefArc(ele._arc.GetSp(),P,ele._arc.GetCp(),
                         ele._arc.R(),ele._arc.ArcRot());
        }
        else{
            arc = DefArc(P,ele._arc.GetEp(),ele._arc.GetCp(),
                         ele._arc.R(),ele._arc.ArcRot());
        }
        newEle.SetArc(arc);
    }*/
                                    ...
                                    double arcLim,bool& isVal)
{
    ...
    case ARCTYPE:{
        if(gocal.TwoPointDistance(ele._arc.GetSp(),
                                  ele._arc.GetEp()) < arcLim)
            return;
        if(ele._arc.R() < arcLim)
            return;
        break;
    }
    case CIRCLETYPE:{
        if(ele._circle.R() < arcLim)
            return;
        break;
    }
    default:
                                   ...
                                   bool isDecNum,int number)
{
    ...
    dis = arc.GetSp().Distance2D(arc.GetEp());
    if(dis > PreErr_2){
        return true;
    }
    gocal.ArcCenterAngle(arc,angle);
    if((dis < endVal && angle < 180) || arc.R() < R ||
                                        ...
                                        double H,double R, bool &isVal)
{
    ...
    if(ele.Type() == LINETYPE){
        if(ele.GetSeg().GetSp().
                Distance2D(ele.GetSeg().GetEp()) < segPre)
            return;
    }
    else if(ele.Type() == ARCTYPE){
        double dis,angle;
        DefArc arc = ele.GetArc();
        arc.OptInitR(PreErr5_2);
        dis = gocal.TwoPointDistance2D(arc.GetSp(),
                                       arc.GetEp());
        gocal.ArcCenterAngle(arc,angle);
    ...
    else if(ele.Type() == CIRCLETYPE){
        if(ele.GetCircle().R() < R)
            return;
    }
    isVal = true;
}
                                         ...
                                         vector<Point>&vPoint)
{
    ...
    for (int i = 0; i < loop.m_vElem.size(); ++i) {
        if(loop.m_vElem[i]._type == LINETYPE){
            vPoint.push_back(loop.m_vElem[i]._segment.GetSp());
        }
        else if(loop.m_vElem[i]._type == ARCTYPE){
            double stepVal = value,angVal;
            DefArc arc = loop.m_vElem[i]._arc;
            if(!isBaseAngle){
                stepVal = matcal.AngleBaseBowHeight(arc.R(),value);
            }
            ...
            if(!isBaseAngle){
                stepVal = matcal.AngleBaseBowHeight(circle.R(),value);
            }
            gocal.DiscreteCircleToPoint(circle.GetCp(),circle.R(),
                                        loop.IsCW(),stepVal,vPoint_);
        }
...
void ElemCalculate::SetLoopOpenClose(DefLoop& loop)
{
    ...
    if(loop.m_vElem.size() == 1 &&
            loop.m_vElem[0]._type == CIRCLETYPE){
        loop.SetEndType(Ofte::EndType::Polygon);
    }
    Point startPt,endPt;
    int len = loop.m_vElem.size() - 1;
    ...
    if(isClose){loop.SetEndType(Ofte::EndType::Polygon);}
    else{loop.SetEndType(Ofte::EndType::Round);}
}
...
void ElemCalculate::ElemCoordDecimalPlaces(int number,DefElem &ele)
{
    ...
    switch (ele.Type()) {
    case LINETYPE:{
        double sx = ele.GetSeg().GetSp().X(),
                sy = ele.GetSeg().GetSp().Y(),
                ex = ele.GetSeg().GetEp().X(),
                ey = ele.GetSeg().GetEp().Y();
    ...
    case ARCTYPE:{
        double sx = ele.GetArc().GetSp().X(),
                sy = ele.GetArc().GetSp().Y(),
                ex = ele.GetArc().GetEp().X(),
                ey = ele.GetArc().GetEp().Y()/**,

        cx = ele.GetArc().GetCp().X(),

        cy = ele.GetArc().GetCp().Y()*/;
                                       ...
                                       bool isOrder, double lenth, Point& res)
{
    ...
    if(isOrder){
        ...
        LoopPointBaseLenth(loop,loop.NexId(id),
        ...
        LoopPointBaseLenth(loop,loop.PreId(id),
                           isOrder,lenth-len,res);
    }
}
...
#endif
    ///如果p 在圆内,直接返回
    if (circle.IsContPt(p,PreErr5_8)){
        return circle;
    }
    ///否则,将p 加入边界点,重新计算.
                                        ...
                                        double err, DefCircle &circle)
{
    if(loop.IsEmpty())
        return;
    if(loop.IndexElem(0).Type() == CIRCLETYPE){
    ...
    {121.8305,-23.1525},{100.5492,-23.1525},{71.1864,-27.998},{83.3086,-10.5005},
    {79.8067,8.0737},{108.0919,7.8045},{114.0184,25.8404},{136.6466,14.2651},
    {140.1485,-30.4207},{191.062,-36.8813},{160.8911,-65.6848},{106.745,-64.8772},
    ...
    {37.5135,6.9969},{55.8315,-37.6889},{-24.1753,-75.9141},{-24.1753,-75.9141}};
    for (int i = 0; i < points.size(); ++i) {
    ...
    /// 初始圆:只包含第一个点
    auto current = DefCircle(points[0], 0);
    for (size_t i = 1; i < points.size(); ++i) {
        /// 如果当前点不在圆内，需要更新圆
        if (current.IsContPt(points[i], PreErr5_10))
            continue;
        /// 新圆初始化为只包含这个点
        current = DefCircle(points[i], 0);
        /// 重新检查前面的所有点
        for (size_t j = 0; j < i; ++j) {
            if (current.IsContPt(points[j], PreErr5_10))
                continue;
            /// 构造包含这两个点的圆
            ...
            for (size_t k = 0; k < j; ++k) {
                if (current.IsContPt(points[k],PreErr5_10))
                    continue;
                /// 构造包含这三个点的圆
                ...
                if (std::abs(G) < PreErr5_10) {
                    auto c1 = DefCircle(a.Midpoint(b), a.Distance2D(b) / 2);
                    auto c2 = DefCircle(a.Midpoint(c), a.Distance2D(c) / 2);
                    auto c3 = DefCircle(b.Midpoint(c), b.Distance2D(c) / 2);
