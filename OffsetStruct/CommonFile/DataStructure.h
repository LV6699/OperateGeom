...
public:
  ...
  double Dot2D(const Point &v) const {
  ...
  double Cross2D(const Point &p) const { return _x * p.Y() - _y * p.X(); }
  ...
  /// 叉积,P^Q>0,P 在 Q 的顺时针方向<0,P 在 Q 的逆时针方向=0,P,Q
  /// 共线,可能同向或反向.
  double operator^(const Point &p) const {
...
public:
  ...
  const Point &SP() const { return _segSp; }
  const Point &EP() const { return _segEp; }
  const Point &SegSp() const { return _segSp; }
  const Point &SegEp() const { return _segEp; }
...
public:
  ...
  DefArc(const Point &s, const Point &e, const Point &c, Ofte::RotDir r)
      : _arcSp(s), _arcEp(e), _arcCp(c), _arcRot(r) {
    IniArcRadius();
    _isCWArc = (r == Ofte::CLOCKWISE) ? true : false;
  }
  DefArc(const Point &s, const Point &e, const Point &c, bool isCW)
      : _arcSp(s), _arcEp(e), _arcCp(c), _isCWArc(isCW) {
    _arcRot = isCW ? Ofte::CLOCKWISE : Ofte::ANTICLOCKWISE;
    IniArcRadius();
  }
  DefArc(const Point &s, const Point &e, const Point &c, bool isCW,
         const Point &n)
      : _arcSp(s), _arcEp(e), _arcCp(c), _isCWArc(isCW), _n(n) {
    _arcRot = isCW ? Ofte::CLOCKWISE : Ofte::ANTICLOCKWISE;
    IniArcRadius();
  }
  DefArc(const Point &s, const Point &e, const Point &c, double r, bool isCW)
      : _arcSp(s), _arcEp(e), _arcCp(c), _arcR(r), _isCWArc(isCW) {
    _arcRot = isCW ? Ofte::CLOCKWISE : Ofte::ANTICLOCKWISE;
  }
  DefArc(const Point &S, const Point &E, const Point &C, double radius,
         Ofte::RotDir r)
  ...
  void SetArcRot(Ofte::RotDir r) {
    _arcRot = r;
    _isCWArc = (r == Ofte::CLOCKWISE) ? true : false;
  }
  void SetArcCW(bool b) {
    _isCWArc = b;
    _arcRot = b ? Ofte::CLOCKWISE : Ofte::ANTICLOCKWISE;
  }
  ...
  void IniArcRadius() { _arcR = GetCalR(); }
  ...
  void Reverse() {
    ...
    _arcEp = p;
    _arcRot =
        _arcRot == Ofte::CLOCKWISE ? Ofte::ANTICLOCKWISE : Ofte::CLOCKWISE;
    _isCWArc = !_isCWArc;
  }
  void PointSplit(const Point &p, bool isChangeEp) {
    if (isChangeEp) {
  ...
  double GetJudgeR(double v = PreErr5_3) const {
    if (_arcR > v) {
      return _arcR;
    }
    return GetCalR();
  ...
  const Point &SP() const { return _arcSp; }
  const Point &ArcEndPt() const { return _arcEp; }
  const Point &EP() const { return _arcEp; }
  const Point &ArcCenterPt() const { return _arcCp; }
  const Point &CP() const { return _arcCp; }
  const Point &ArcSp() const { return _arcSp; }
  const Point &ArcEp() const { return _arcEp; }
  const Point &ArcCp() const { return _arcCp; }
  double ArcRadius() const { return _arcR; }
  double R() const { return _arcR; }
  RotDir ArcRot() const { return _arcRot; }
  bool IsCW() const { return _arcRot == Ofte::CLOCKWISE; }
...
public:
  ...
  void SetCirCP(const Point &p) { _cirCp = p; }
  void SetCirR(double r) { _cirR = r; }
  void SetCirCW(bool v) { _isCW = v; }
  void SetEndptZ(double z) { _cirCp.SetZ(z); }
  void SetNormal(const Point &n) { _n = n; }
  const Point &CirCP() const { return _cirCp; }
  const Point &CP() const { return _cirCp; }
  const Point &Normal() const { return _n; }
  Point UpPoint() const {
...
public:
  OftBasePara(double r, double v) : _toolRadius(r), _offsetVal(v) {
    _isInitail = true;
  }
...
public:
  ...
  void ChangeEndpt(const Point &p, bool isChangeEp) {
    switch (_elemType) {
    case Ofte::LINETYPE:
      _segment.PointSplit(p, isChangeEp);
      break;
    case Ofte::ARCTYPE:
      _arc.PointSplit(p, isChangeEp);
      break;
    default:
      break;
    }
  ...
  const DefSeg &ElemSeg() const { return _segment; }
  const DefArc &ElemArc() const { return _arc; }
  const DefCircle &ElemCircle() const { return _circle; }
  ...
  const Point &StarPt() const {
    switch (_elemType) {
    case Ofte::LINETYPE:
      return _segment.SegStartPt();
      break;
    case Ofte::ARCTYPE:
      return _arc.ArcStartPt();
      break;
    default:
      return _circle._cirCp;
    }
  }
  const Point &EndPt() const {
    switch (_elemType) {
    case Ofte::LINETYPE:
      return _segment.SegEndPt();
      break;
    case Ofte::ARCTYPE:
      return _arc.ArcEndPt();
      break;
    default:
      return _circle._cirCp;
    }
  ...
  bool IsEndpoint(const Point &p, double e) const {
    switch (_elemType) {
    case Ofte::LINETYPE:
      return p.IsSamePoint2D(_segment._segSp, e) ||
             p.IsSamePoint2D(_segment._segEp, e);
    case Ofte::ARCTYPE:
      return p.IsSamePoint2D(_arc._arcSp, e) || p.IsSamePoint2D(_arc._arcEp, e);
    default:
      return p.IsSamePoint2D(_circle._cirCp, e);
    }
  }
  bool IsSameCurve(const DefElem &c, double e) const {
    if (_elemType != c._elemType) {
      return false;
    }
    if (_elemType == OffsetEnum::LINETYPE) {
      return _segment._segSp.IsSamePoint2D(c._segment._segSp, e) &&
             _segment._segEp.IsSamePoint2D(c._segment._segEp, e);
    } else if (_elemType == OffsetEnum::ARCTYPE) {
      return _arc._arcRot == c._arc._arcRot &&
    ...
    } else if (_elemType == OffsetEnum::CIRCLETYPE) {
...
public:
  ...
  void SetInt1XY(double x, double y) {
    _intPnt1.SetX(x);
    _intPnt1.SetY(y);
  }
  void SetInt2XY(double x, double y) {
    _intPnt2.SetX(x);
    _intPnt2.SetY(y);
  }
  void SetIntXY(double x, double y) {
    SetInt1XY(x, y);
    SetInt2XY(x, y);
  }
  void SetExistAll() {
  ...
  void SetIntPt(const Point &p) {
    _intPnt1 = p;
    _intPnt2 = p;
  }
  void SetIntPts(const Point &p1, const Point &p2) {
    _intPnt1 = p1;
    _intPnt2 = p2;
  }
  void SetNotInt() {
...
public:
  ...
  GenerateParam(double r, double v) : _toolRadius(r), _offsetValue(v) {}
...
public:
 ...
 产生的,那么该环就是边界还,m_isMatchTypeTrans 就是用来表示与一个岛屿环配对的边
...
public:
  ...
  DefLoop(const std::vector<DefElem> &v, ProfType ct)
      : m_vElem(v), _loopType(ct) {}
  ...
  DefLoop(const std::vector<Point> &ps, bool isClose = true) {/**
    auto l = ps.size();
    if (l < 2) {return;}
    if (isClose) {for (size_t i = 0; i < l; ++i) {
        DefSeg s(ps[i], ps[pnum::NexId(l, i)]);
        m_vElem.push_back(DefElem(Ofte::LINETYPE, s));
      }return;
    }for (size_t i = 0; i < l - 1; ++i) {
      DefSeg s(ps[i], ps[i + 1]);
      m_vElem.push_back(DefElem(Ofte::LINETYPE, s));
    }*/
  }
  ...
  void SetOffsetType(Ofte::ToolPathType t) { _loopLabel._loPathType = t; }
  void InitialRadius() {
    for (auto &e : m_vElem) {
      if (e.ElemeType() == Ofte::ARCTYPE) {
        e._arc.IniArcRadius();
      }
    }
  }
  ...
  void SetClipDele(bool v) { _loopLabel._hasClipDel = v; }
  ...
  void IniDir() {
    _isOutward = _loopType == Ofte::OUTSIDEPROFILE ? true : false;
  }
  void SetOutward(bool v) { _isOutward = v; }
  ...
  void SetIsBoundary(bool b) { _loopLabel._isBoundary = b; }
  void Erase(size_t i) { m_vElem.erase(m_vElem.begin() + i); }
  void Reserver(size_t i) { m_vElem.reserve(i); }
  const std::vector<DefElem> &Elems() { return m_vElem; }
  ...
  const DefElem &FirstElem() const { return m_vElem.front(); }
  const DefElem &LastElem() const { return m_vElem.back(); }
  size_t ElemNum() const { return m_vElem.size(); }
  ...
  bool IsOutward() const { return _isOutward; }
  ...
  Ofte::MovePathType MoveType() const { return _moveType; }
  Ofte::ToolPathType OffsetType() const { return _loopLabel._loPathType; }
  const Point &CurveEndpt(size_t i, bool isEp) const {
    return m_vElem[i].Endpoint(isEp);
  }
  const Point &Endpoint(bool isEp) const {
    if (isEp) {
      return m_vElem.back().Endpoint(true);
    }
    return m_vElem.front().Endpoint(false);
  }
  bool IsEndToEnd(double v, bool is2d = true) const {
    if (is2d) {
      return Endpoint(false).IsSamePoint2D(Endpoint(true), v);
    }
    return Endpoint(false).IsSamePoint3D(Endpoint(true), v);
  }/**
  bool IsConnect(double err, bool isOpen = false, bool is2D = true) const {
    if (m_vElem.empty())
      return false;
    ...
    for (size_t i = 0; i < s; ++i) {
      n = NexId(i);
      if (is2D) {
        if (!m_vElem[i].EndPt().IsSamePoint2D(m_vElem[n].StarPt(), err)) {
          return false;
        }
      } else {
        if (!m_vElem[i].EndPt().IsSamePoint3D(m_vElem[n].StarPt(), err)) {
          return false;
        }
      }
    }
  ...
  bool IsBoundary() const { return _loopLabel._isBoundary; }
  bool IsSameCurves(const std::vector<DefElem> &cs, double e) const {
    if (m_vElem.size() != cs.size() || IsEmpty()) {
      return false;
    }
    size_t s = 0;
    bool is = false;
    ...
    for (size_t i = 0; i < m_vElem.size(); ++i) {
      if (!m_vElem[i].IsSameCurve(cs[i], e)) {
        return false;
      }
    }
    return true;
  }
  bool HasClipDele() const { return _loopLabel._hasClipDel; }
  bool IsCircleLoop() const {
    return Size() == 1 && m_vElem[0]._elemType == Ofte::CIRCLETYPE;
  }
...
public:
  ...
  void ReverseNodes() {
    std::reverse(_vOffsetNode.begin(), _vOffsetNode.end());
  }
  ...
  std::shared_ptr<OffsetNode> LastOftNode() const {
    return _vOffsetNode.back();
  }
  DefLoop IndexLoop(int i) const { return _vLoopData[i]; }
  std::shared_ptr<OffsetNode> IndexAreaNode(int i) const {
    return _vOffsetNode[i];
  }
  std::vector<DefLoop> ArrayLoop() const { return _vLoopData; }
  std::vector<std::shared_ptr<OffsetNode>> ArrayNode() const {
    return _vOffsetNode;
  }
...
public:
  InOutLoop() {}
  InOutLoop(const std::vector<DefLoop> &in, const std::vector<DefLoop> &out)
...
public:
  PathTechnic() {}
  PathTechnic(bool isBlank, bool isAccOut, bool isBndOft, OffsetStyle style,
              double angle)
      : _isBlankBnd(isBlank), _isAcceptBorOut(isAccOut), _isBndOffset(isBndOft),
        _profOffsetStyle(style), _styleAngle(angle) {}
...
public:
  OffsetUnit() {}
  OffsetUnit(bool isFixOffset, int offsetNum, double toolRadius,
             double firstOffsetVal, double offsetVal, bool isSmooth,
             double smoothVal)
      : _isOftFixNum(isFixOffset), _offsetNum(offsetNum),
        _toolRadius(toolRadius), _firOffsetVal(firstOffsetVal),
        _offsetVal(offsetVal), _isSmooth(isSmooth), _smoothVal(smoothVal) {}
...
public:
  PrepOption() {}
  PrepOption(double oft, bool delInv, bool opeSup, bool ete, bool close,
             bool ablPre = true, bool absPre = false)
...
public:
  ...
  OffsetParam(bool isFixOftNum, int offsetNum, double firOffsetVal,
              double offsetVal, bool isSmooth, double smoothVal) {
    OffsetUnit oftUnit(isFixOftNum, offsetNum, firOffsetVal, offsetVal,
                       isSmooth, smoothVal);
    _offsetUnit = oftUnit;
  }
  /// 带岛屿轮廓的偏置参数设置
  ...
  OffsetParam(bool isFix, int oftNum, double oftVal) {
...
public:
  ...
  GeomArea(const DefLoop &b, const std::vector<DefLoop> &v, bool isBOft,
           bool isIOft)
  ...
  void ClearCurves() {
    _bndLoop.ClearElem();
    _oriBndLoop.ClearElem();
    for (auto &l : _vIslLoop) {
      l.ClearElem();
    }
    for (auto &l : _vOriIslLoop) {
      l.ClearElem();
    }
    for (auto &l : _vBlockLoop) {
      l.ClearElem();
    }
  }
  void ClearLoop() {
...
public:
  ...
  void PushBackArea(const GeomArea &area) { _vGeomArea.push_back(area); }
  ...
  size_t GeomAreaNum() const { return _vGeomArea.size(); }
  size_t AreaNodeNum() const { return _vAreaNode.size(); }
  size_t WindNumber() const { return _windNum; }
...
public:
  ...
  DefElem m_curValPart, m_curInValPart, m_anoValPart, m_anoInValPart;
};
...
public:
  ...
  CylinderHelix(const Point &c, double r, double h, double angle, bool isRight)
      : _helCenterPt(c), _helRadius(r), _helHeight(h), _helAngle(angle),
        _isRightHelix(isRight), _hasInitHelix(true) {}
...
public:
  ...
  void SetLoc(const Point &loc) { _loc = loc; }
  void SetDir(const Point &dir) { _dir = dir; }
...
public:
  ...
  CheckElem(const DefElem &e, int i, const std::vector<CheckPoint> &vPt,
      ...
      : _element(e), _eleIndex(i), _vCheckPnt(vPt), _vDistance(vDis),
        _checkDis(dis) {}
...
public:
  ...
  CheckLoop(const DefLoop &loop, double ref) : _loop(loop), _refValue(ref) {}
  void SetMaxError() {
...
public:
  ...
  IndexDist(int i, int toi, double d) : _index(i), _toIndex(toi), _distance(d) {}
