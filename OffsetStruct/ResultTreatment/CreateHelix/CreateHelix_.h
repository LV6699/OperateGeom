...
/// 侧,则是左螺旋,否则是右螺旋.
void CreateHelix::LoopSpecifyPositionHelix(const GeomArea& oriLoop,
                                           const DefLoop& loop,
                                           ...
                                           DefLoop& resLoop)
{
    ...
    helsol.HelixRadiusInterfere(oriLoop,element,posPnt,
                                oftPara._offsetUnit.ToolRadius(),
                                isCpRightPos,loopRot,helix);
                                    ...
                                    DefLoop& resLoop)
{
    ...
    vector<Point>vPoint;
    HelixCurveSolve helsol;
    helCenter.SetZ(posPnt.Z());
    helsol.GenerateSpecifyHelixPoint(helCenter,helRadius,startAngle,                                     perAngHeight,totalAngle,
                                     isRightHelix,vPoint);
    if(vPoint.size() < 2)
        return;
