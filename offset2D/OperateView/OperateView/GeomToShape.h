...
public:
    ...
    gp_Pnt ToOccPt(const Point& p){return gp_Pnt (p.X(),p.Y(),p.Z());}
    void PointToShape(const Point& p,TopoDS_Shape &shape)
    {
        BRepBuilderAPI_MakeVertex v(ToOccPt(p));
        shape = v;
    }
    void SegmentToShape(const Point& sp,const Point& ep,
                        TopoDS_Shape &shape)
    {
    ...
    void ArcToShape(const DefArc& arc_,
                    TopoDS_Shape &shape)
    {
        auto arc = arc_;
        ...
        ///创建圆弧段
        Handle(Geom_TrimmedCurve) arc__ = GC_MakeArcOfCircle(circle, u1, u2, !isCW);
        TopoDS_Edge edge = BRepBuilderAPI_MakeEdge(arc__);///创建拓扑边
#endif
        shape = edge;
    }
    void CircleToShape(const DefCircle& circle,
                       TopoDS_Shape &shape)
    {
    ...
    void LoopToShape(const DefLoop& loop,TopoDS_Shape &shape);
    void LoopsToShape(const std::vector<DefLoop> &loops, TopoDS_Shape &shape);
    void GeomAreaToShape(const GeomArea& area,TopoDS_Shape& shape);
};
#endif
#endif // GEOMTOSHAPE_H
